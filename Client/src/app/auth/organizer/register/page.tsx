...
import { Eye, EyeOff, Loader2, Building, User, Mail, Lock, ArrowLeft, CheckCircle } from 'lucide-react';
import { validateEmail, validatePassword, validateName, validatePasswordMatch } from '@/utils/validation';

export default function OrganizerRegisterPage() {
  const { registerOrganizer, loading, clearAuthError } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await registerOrganizer(formData.email.trim(), formData.password, fullName);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary/10 to-accent/10 flex items-center justify-center py-6 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full">
        <Link 
          href="/auth/login" 
          className="inline-flex items-center text-foreground/80 hover:text-foreground mb-4 transition-colors"
        >
        ...
        <div className="bg-card rounded-xl shadow-3xl p-6">
          <div className="text-center mb-6">
            ...
            <p className="mt-1 text-muted-foreground text-sm">
              Start managing conferences with ease
            </p>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="flex items-start text-sm">
              ...
              <label htmlFor="terms" className="ml-3 text-muted-foreground">
                I agree to the{' '}
                <Link href="/terms" className="text-primary hover:text-primary/90 underline">Terms of Service</Link>{' '}
                and{' '}
                <Link href="/privacy" className="text-primary hover:text-primary/90 underline">Privacy Policy</Link>
              </label>
            </div>
          ...
          <div className="mt-6 text-center space-y-3">
            <p className="text-sm text-muted-foreground">
              ...
              <Link href="/auth/organizer/login" className="font-medium text-primary hover:text-primary/90 transition-colors">
                Sign in here
              </Link>
            </p>
            <div className="pt-3 border-t border-border">
              <p className="text-xs text-muted-foreground mb-2">Looking to attend events?</p>
              <Link href="/auth/login" className="inline-flex items-center px-3 py-1.5 text-sm font-medium text-primary bg-primary/10 hover:bg-primary/20 rounded transition-colors">
